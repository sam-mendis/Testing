...
class k2400:
    ...
    def __del__(self):
        ...
        try:
            ...
            # wait 200ms for this command to execute before closing the interface
            time.sleep(0.2)
    ...
    def _getSourceMeter(self, rm):
        ...
        if self.idnContains in self.idn:
            ...
            raise ValueError(
                "Got a bad response to *IDN?: {:s}".format(self.idn))
    ...
    def _setupSourcemeter(self, twoWire, front):
        ...
        # the instrument will never generate over 20v
        sm.write('source:voltage:protection 20')
        ...
        # we'll do this once before every measurement
        sm.write(':system:azero off')
        ...
        if self.sm.query(':system:rsense?') == "1":
            ...
            if "CONTACT-CHECK" in opts.upper():
                if value == True:
                    ...
                    self.setupDC(sourceVoltage=False, compliance=3,
                                 setPoint=0, senseRange='f', auto_ohms=False)
                    ...
                    self.opc()
                    self.sm.write(
                        f':sense:voltage:nplcycles {self.nplc_user_set}')
    ...
    def _flush_input_buffer(self, sm, delayms=0):
        try:
            # that's a pyserial object
            session = sm.visalib.sessions[sm._session]
        ...
        if self.sm.interface_type == pyvisa.constants.InterfaceType.asrl:
            ...
            if self.four88point1 == True:
                ...
                vals = self.sm.read_binary_values(
                    data_points=nPoints*m_len, is_big_endian=True)
            else:
                vals = self.sm.query_binary_values(
                    ':read?', data_points=nPoints*m_len)
    ...
    def contact_check(self):
        ...
        if self.sm.query(':output?') == "1":  # check if that worked
            self.sm.write("INIT")
            # TODO: figure out a better way to do this. mysterious dealys = bad
            time.sleep(0.1)
...
if __name__ == "__main__":
    ...
    measurement_datatype = np.dtype({'names': ['voltage', 'current', 'resistance', 'time', 'status'], 'formats': [
                                    'f', 'f', 'f', 'f', 'u4'], 'titles': ['Voltage [V]', 'Current [A]', 'Resistance [Ohm]', 'Time [s]', 'Status bitmask']})
    ...
    # create a custom data type to hold our data
    measurement_datatype = np.dtype({'names': ['voltage', 'current', 'time', 'status'], 'formats': [
                                    'f', 'f', 'f', 'u4'], 'titles': ['Voltage [V]', 'Current [A]', 'Time [s]', 'Status bitmask']})
    ...
    startV = 0
    endV = 1
    k.setupSweep(compliance=0.01, nPoints=numPoints,
                 start=startV, end=endV)  # set the sweep up
    t0 = time.time()
